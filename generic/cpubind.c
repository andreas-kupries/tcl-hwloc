...
static int parse_set_options  (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags);
static int parse_get_options  (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags);
static int parse_last_options (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags);
...
int parse_cpubind_args (topo_data* data, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
    ...
    switch (index) {
    case CPUBIND_SET:
	at = parse_set_options (interp, objc, objv, &pid, &flags);
	if (at < 0) {
	    return TCL_ERROR;
	}
	...
	at = parse_get_options (interp, objc, objv, &pid, &flags);
	if (at < 0) {
	    return TCL_ERROR;
	}
    ...
    case CPUBIND_LAST:
	at = parse_last_options (interp, objc, objv, &pid, &flags);
	if (at < 0) {
	    return TCL_ERROR;
	}
...

/*
 * Helper functions for parse_cpubind_args below. Processes all the options
 */

static int
parse_set_options (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags)
{
    ...
    enum options {
        CPUBIND_SET_NOMEMBIND, CPUBIND_SET_PID, CPUBIND_SET_PROCESS,
	CPUBIND_SET_STRICT,    CPUBIND_SET_THREAD
    };
    static int map [] = {
...
static int
parse_get_options (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags)
{
    static const char* options[] = {
	"-pid",    "-process",
	"-strict", "-thread", NULL
    ...
    enum options {
        CPUBIND_PID,    CPUBIND_PROCESS,
	CPUBIND_STRICT, CPUBIND_THREAD
    };
    static int map [] = {
	0,                    HWLOC_CPUBIND_PROCESS,
	HWLOC_CPUBIND_STRICT, HWLOC_CPUBIND_THREAD
    };
...
static int
parse_last_options (Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[], int* pid, int* flags)
{
    static const char* options[] = {
        "-pid", "-process", "-thread", NULL
    };
    enum options {
        CPUBIND_PID, CPUBIND_PROCESS, CPUBIND_THREAD
    };
    static int map [] = {
	0, HWLOC_CPUBIND_PROCESS, HWLOC_CPUBIND_THREAD
    };
